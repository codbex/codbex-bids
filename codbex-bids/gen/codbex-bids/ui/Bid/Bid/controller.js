angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-bids-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Bid" && e.view === "Bid" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Bid", `Unable to list/filter Bid: '${response.message}'`);
						return;
					}
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("Bid-filter", {
