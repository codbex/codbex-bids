	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', function ($scope, messageHub, entityApi) {
		...
		if (window != null && window.frameElement != null && window.frameElement.hasAttribute("data-parameters")) {
			...
			if (dataParameters) {
				...
				$scope.optionsOffer = params.optionsOffer;
				...
				$scope.optionsBidStatus = params.optionsBidStatus;
			}
		}
		...
		$scope.create = function () {
			...
			entityApi.create(entity).then(function (response) {
				if (response.status != 201) {
					$scope.errorMessage = `Unable to create Bid: '${response.message}'`;
					return;
				}
		...
		$scope.update = function () {
			...
			entityApi.update(id, entity).then(function (response) {
				if (response.status != 200) {
					$scope.errorMessage = `Unable to update Bid: '${response.message}'`;
					return;
				}
		...
		$scope.cancel = function () {
			$scope.entity = {};
			$scope.action = 'select';
			messageHub.closeDialogWindow("Bid-details");
		};
